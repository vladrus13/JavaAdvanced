...
public class CrawlerHardTest extends CrawlerEasyTest {
    ...
    private static void testPerformance(final int perHost, final double target) throws IOException {
        allItmo(perHost);
        final long time = allItmo(perHost);
        System.err.println("Time: " + time);
        Assert.assertTrue("Too parallel: " + time, time > 0.8 * target);
        Assert.assertTrue("Not parallel: " + time, time < 1.2 * target);
    }

    private static long allItmo(final int perHost) throws IOException {
        return test("https://itmo.ru", 2, Integer.MAX_VALUE, Integer.MAX_VALUE, perHost, 100, 50);
    }
}
